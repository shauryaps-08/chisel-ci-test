...
class PipelineCpu extends Module {
  ...
  val instrMem = VecInit(Seq(
    ...
    Cat(0.U(3.W), 4.U(3.W), 4.U(3.W), 5.U(3.W), 0.U(20.W)),   // ADD  r5, r4, r4 (uses r4 immediately -> forwarding test)
  ...
  val opB = Mux(id_ex_op === 7.U, id_ex_imm, opBraw) // ADDI uses immediate
  ...
  val aluResult = MuxLookup(id_ex_op, 0.U)(Seq(
    ...
    2.U -> (opA & opB),
    3.U -> (opA | opB),
    4.U -> (opA + id_ex_imm), // LOAD address = base + imm
    5.U -> (opA + id_ex_imm), // STORE address = base + imm
    7.U -> (opA + opB)        // ADDI
  ))
  ...
  io.debugRegOut := regFile.read(6.U) // expose r6 for the test to check
}
